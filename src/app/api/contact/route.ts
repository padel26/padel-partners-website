...
export async function POST(request: NextRequest) {
  try {
    ...
    await resend.emails.send({
      ...
      replyTo: email,
      subject: `New ${type === "quote" ? "Quote Request" : "Enquiry"} from ${name}`,
      html: `
        <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 24px;">
          <div style="background: #1A6B3A; padding: 16px 24px; border-radius: 8px 8px 0 0;">
            <h1 style="color: white; margin: 0; font-size: 18px;">New ${type === "quote" ? "Quote Request" : "Website Enquiry"}</h1>
          </div>
          <div style="background: #F4F4F2; padding: 24px; border-radius: 0 0 8px 8px;">
            <table style="width: 100%; border-collapse: collapse;">
              ...
              ${company ? `<tr><td style="padding: 8px 0; font-weight: 600; color: #141414;">Company</td><td style="color: #141414;">${company}</td></tr>` : ""}
              <tr><td style="padding: 8px 0; font-weight: 600; color: #141414;">Type</td><td style="color: #141414;">${type}</td></tr>
            </table>
